...
from amplifier_core import AgentContext
from amplifier_core import AgentResult
...
class ZenArchitectAgent:
    ...
    async def execute(self, task: dict[str, Any], context: AgentContext) -> AgentResult:
        ...
        try:
            ...
            if not providers:
                return AgentResult(success=False, error={"message": "No providers available"})
            ...
            response = await provider.complete(messages, temperature=self.temperature, max_tokens=self.max_tokens)

            # Return the design
            return AgentResult(
                success=True,
                text=response.content,
                data={"task": "architecture_design", "principles_applied": self.principles[:3]},
            )
...
Your responses should be clear, concise, and focused on practical simplicity."""
    ...
    def _build_prompt(self, requirements: str, constraints: list, existing_system: str) -> str:
        """Build the design prompt."""
        prompt_parts = []

        prompt_parts.append("Design a system architecture for the following requirements:")
